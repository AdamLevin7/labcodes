...
class App:
	def __init__(self, window, window_title):
		...
		self.btn_bkgrnd = tk.Button(self.window,text="Subtract Background",width=50,command=self.sub_back)
		...
		self.btn_bkgrnd.grid(row=5,column=0)
	...
	def pack_buttons(self):
		...
		self.btn_convert.grid(row=6, column = 0)
		self.btn_plot.grid(row=7, column = 0)
		self.btn_expPos.grid(row=8, column = 0)
		self.btn_expVid.grid(row=9, column = 0)
		self.btn_restart.grid(row=10, column=0)
		self.btn_quit.grid(row=11, column = 0) 
		if self.flag_bckgrnd == False:
			self.btn_bkgrnd.grid(row=5,column=0)
	...
	def color_space(self):
		...
		#Declare Buttons 
		#btn_rg = (y_hist,cr_hist,cb)
		#btn_rb = (y_hist,cr_hist,cb_hist)
	...
	def sub_back(self):
		self.flag_bckgrnd = True
		self.unpack_buttons()
		self.btn_bkgrnd.grid_remove()
		self.text = tk.Label(text="Subtacting Background")
		self.text.grid()
		self.vid.subBackground()
		self.text.grid_remove()
		self.vid = Vid('subback.mp4')
		self.pack_buttons()
	...
	def quit(self):
		if self.flag_color == True:
			self.vid.__del__()
			os.remove("color.mp4")
		if self.flag_bckgrnd == True:
			self.vid.__del__()
			os.remove('subback.mp4')
		self.window.destroy()
...
class Vid:
	...
	def cnvtColorSpace(self):
		...
		while True:
			...
			if ret == True:
				...
				#------------------------------------------------------------------
				#Rearrange order and type of y, cr, and cb to get different results
				#Possible values to rearrange: 
				#	y, y_hist
				# 	cr, cr_hist
				#	cb, cb_hist 
				channels = (y_hist,y_hist,y_hist)
				color = cv2.merge(channels)
				#------------------------------------------------------------------

				color = cv2.cvtColor(color,cv2.COLOR_YCrCb2BGR)
				med = cv2.cv2.bilateralFilter(color, 15, 75, 75)
				out.write(med)
	...
	def subBackground(self):
		out = cv2.VideoWriter('subback.mp4',0x7634706d,self.fps,(self.width,self.height))
		backSub = cv2.createBackgroundSubtractorMOG2()
		while True:
			ret,frame = self.get_frame()
			if ret == True:
				fgMask = backSub.apply(frame)
				mask_three = cv2.cvtColor(fgMask,cv2.COLOR_GRAY2BGR)
				out.write(mask_three)
				if cv2.waitKey(25) & 0xFF == ord('q'): 
					break
			else:
				break
