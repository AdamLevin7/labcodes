        ...
        if ret == True:
            ...
            if framenum in range(int(data['frame'].iloc[0]),int(data['frame'].iloc[-1])):
                ...
                for cnts in range(len(segments)):
                    ...
                    if (any(np.isnan(data.filter(regex = segments['origin'][cnts]).loc[framenum,:])) or
                        any(np.isnan(data.filter(regex = segments['other'][cnts]).loc[framenum,:]))):
                        pass
                    else:
                        # if it is head
                        if (segments.iloc[cnts,:]).name == 'head':
                            # origin
                            orig_loc = tuple(data.filter(regex = segments['origin'][cnts]).loc[framenum,:].astype(int))
                            # other
                            oth_loc = tuple(data.filter(regex = segments['other'][cnts]).loc[framenum,:].astype(int))
                            ...
                            segcm_loc = tuple(segcm.loc[framenum,:].astype(int))
                        ...
                        # if it is trunk
                            ...
                            oth_loc = tuple(oth.loc[framenum,:].astype(int))
                            # draw segment with red line
                            frame = cv2.line(frame, orig_loc, oth_loc, (0,0,255), thickness=3)
                            # segment center of mass
                            segcm = data_cm.filter(regex = (segments.iloc[cnts,:]).name)
                        ...
                        # if another segment
                            ...
                            if len(orig.columns) > 0:
                                ...
                                segcm_l = tuple(segcm.filter(regex = 'left').loc[framenum,:].astype(int))
                                segcm_r = tuple(segcm.filter(regex = 'right').loc[framenum,:].astype(int))
                            # other
                            oth = data.filter(regex = segments['other'][cnts])
                            # find if location exists in digitized data set
                            ...
                            if (len(orig.columns)>0 and len(oth.columns)>0):
                                ...
                                if (len(orig_l.columns)>0 or len(oth_l.columns)>0):
                                    # create tuple
                                    orig_loc = tuple(orig_l.loc[framenum,:].astype(int))
                                    oth_loc = tuple(oth_l.loc[framenum,:].astype(int))
                                    # draw segment with red line
                                    frame = cv2.line(frame, orig_loc, oth_loc, (0,0,255), thickness=3)
                                    # display segment center of mass location
                                ...
                                if (len(orig_r.columns)>0 or len(oth_r.columns)>0):
                                    # create tuple
                                    orig_loc = tuple(orig_r.loc[framenum,:].astype(int))
                                    oth_loc = tuple(oth_r.loc[framenum,:].astype(int))
                                    # draw segment with red line
                                    frame = cv2.line(frame, orig_loc, oth_loc, (0,0,255), thickness=3)
                                    # display segment center of mass location
                                ...
                                if (len(orig_l.columns)==0) and (len(orig_r.columns)==0):
                                    # create tuple
                                    orig_loc = tuple(orig.loc[framenum,:].astype(int))
                                    oth_loc = tuple(orig.loc[framenum,:].astype(int))
                                    # draw segment with red line
                                    frame = cv2.line(frame, orig_loc, oth_loc, (0,0,255), thickness=3)
                                    # display segment center of mass location
                                    segcm_loc = tuple(segcm.loc[framenum,:].astype(int))
                ...
                if not any(np.isnan(data_cm[['body_x','body_y']].loc[framenum])):
                    # create tuple
                    bodycm_loc = tuple(data_cm.filter(regex = 'body').loc[framenum,:].astype(int))
